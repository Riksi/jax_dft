...
class OnpUtilsTest(absltest.TestCase):
    ...
    def test_exact_h_atom_density_wrong_shape(self):
        ...
        with self.assertRaisesRegex(
            ...
            np_utils._get_exact_h_atom_density(
                grids, dx
            )

    def test_spherical_superposition_density(self):
        # Calls _get_exact_h_atom_density([grids, grids - 2], 0.02)
        #  to get densities
        # Then returns densities[0] * 1 + densities[1] * 2
        #  (charges dotted with densities)
        ...
        # sum(density) * dx
        # = sum((densities[0] * 1 + densities[1] * 2)) * dx
        # = sum(densities[0] * dx)* 1 + sum(densities[1] * dx) * 2
        # = 1 * 1 + 1 * 2
        # = 3
        np.testing.assert_allclose(
            np.sum(density) * 0.02, 3.
        )
